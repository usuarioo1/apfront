...
export default function ListaDeCadenas() {
    ...
    return (
        <div className="p-4 bg-gray-100 min-h-screen">
            <h1 className="text-3xl font-bold mb-8 text-center text-gray-800">Lista de Cadenas</h1>
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {cadenas.map((cadena) => (
                    <div key={cadena._id} className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg">
                        ...
                        <div className="p-4">
                            <h2 className="text-lg font-semibold text-gray-800 mb-2 text-center">{cadena.name}</h2>
                            <p className="text-center text-gray-600">Precio: ${cadena.precio}</p>
                            <p className="text-center text-gray-600 mb-2">Stock: {cadena.stock}</p>
                            <button
                                className="w-full bg-blue-600 text-white text-sm py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors duration-300"
                                onClick={() => handleAddToCart(cadena)} // Llamamos a handleAddToCart cuando el usuario hace clic en el botón
                            >
                                Añadir al Carrito
                            </button>
                        </div>
