    ...
    return (
        <div className="p-4 bg-gray-100 min-h-screen">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {aros.map((aro) => {
                    const precioMayor = parsePrice(aro.precio_por_mayor)
                    const precioDetalle = parsePrice(aro.precio) ?? 0
                    const precioMayorTexto = `$${Math.round(precioMayor ?? precioDetalle)}`

                    return (
                        <div key={aro._id} className="rounded-2xl border border-zinc-200 bg-white overflow-hidden flex flex-col h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                            <Link href={`/aros/${aro._id}`} className="block">
